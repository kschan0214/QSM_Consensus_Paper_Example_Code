%% SEPIA_SIEMENS_Bipolar_config.m
%
% Objective:
...
% 
% Data required: 
%   (1) SEPIA-ready multi-echo GRE data from SIEMENS, Bipolar readout
...
input = struct();
input(1).name   = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','Pipeline_Standard','sub-001_ses-SIEMENS_acq-Bipolar_localfield.nii.gz');
input(2).name   = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','sub-001_ses-SIEMENS_acq-Bipolar_part-mag_GRE.nii.gz');
input(3).name   = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','Pipeline_Standard','sub-001_ses-SIEMENS_acq-Bipolar_weights.nii.gz'); 
input(4).name   = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','sepia_header.mat');
output_basename = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','Pipeline_Alternative1','sub-001_ses-SIEMENS_acq-Bipolar');
mask_filename   = fullfile(work_dir,'derivatives','SEPIA','SIEMENS','Bipolar','GRE','Pipeline_Standard','sub-001_ses-SIEMENS_acq-Bipolar_mask_QSM.nii.gz');
